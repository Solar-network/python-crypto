...
class HtlcClaimDeserializer(BaseDeserializer):
    def deserialize(self):
        ...
        lock_transaction_id = hexlify(self.serialized[starting_position + offset : starting_position + offset + 32])
        offset += 32

        unlock_secret_length = read_bit8(self.serialized, offset=starting_position + offset)
        offset += 1

        unlock_secret = hexlify(self.serialized[starting_position + offset : starting_position + offset + unlock_secret_length])
